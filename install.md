# missing-data-imputacion guide installation
...
activate missing_data_imputacion
```
...
To move beyond notebook prototyping, all reusable code should go into the `missing_data_imputacion/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `missing_data_imputacion` folder and use the modules inside your notebooks :
...
```python
from missing_data_imputacion.utils.paths import data_dir
data_dir()
```
